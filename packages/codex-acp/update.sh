...
REPO_ROOT="$(realpath "$PACKAGE_DIR/../..")"
...
update_package_nix() {
  ...
  awk \
    ...
    /version = "[0-9.]+";/ {
      version_count++
      sub(/version = "[0-9.]+";/, "version = \"" latest_version "\";")
    }
    /hash = "sha256-[^"]+";/ && src_hash_count == 0 {
      src_hash_count++
      sub(/hash = "sha256-[^"]+";/, "hash = \"" src_hash "\";")
    }
    { print }
    END {
      if (comment_count != 1 || rev_count != 1 || codex_hash_count != 1 || version_count != 1 || src_hash_count != 1) {
        print "Failed to update one or more package.nix fields" > "/dev/stderr"
        exit 1
      }
    }
  ' "$PACKAGE_NIX" >"$tmp"
...
current_version="$(nix eval --raw .#codex-acp.version)"
...
update_package_nix
write_librusty_v8_nix
nix-update codex-acp --flake --version skip
